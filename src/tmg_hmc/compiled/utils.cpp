...
double* calc_all_solutions(double q1, double q2, double q3, double q4, double q5) {
    ...
    double base_term = -0.5 * (q1 * q2 + q4 * q5) / (pow(q1, 2) + pow(q4, 2));
    
    // Calculate the first sqrt expression (same for all functions)
    double term1_a = pow(q1*q2 + q4*q5, 2) / pow(pow(q1, 2) + pow(q4, 2), 2);
    double term2_a = (2 * (pow(q2, 2) + 2*q1*q3 - pow(q4, 2) + pow(q5, 2))) / (3. * (pow(q1, 2) + pow(q4, 2)));
    
    // Complex cubic root calculation
    double cubic_numerator = -12*(q2*q3 - q4*q5)*(q1*q2 + q4*q5) + 
    ...
    double big_term = 108*(pow(q1, 2) + pow(q4, 2))*pow(q2*q3 - q4*q5, 2) + 
    ...
    double inner_sqrt = sqrt(-4 * pow(cubic_numerator, 3) + pow(big_term, 2));
    
    // Complete cubic term
    double big_term_plus_sqrt = big_term + inner_sqrt;
    double cubic_root = pow(big_term_plus_sqrt, (1./3.));
    
    double term3_a = (pow(2, (1./3.)) * cubic_numerator) / (3. * (pow(q1, 2) + pow(q4, 2)) * cubic_root);
    double term4_a = cubic_root / (3. * pow(2, (1./3.)) * (pow(q1, 2) + pow(q4, 2)));
    
    // Calculate the complete first sqrt expression
    double first_sqrt_expr = term1_a - term2_a + term3_a + term4_a;
    double first_sqrt = sqrt(first_sqrt_expr);
    
    // Intermediate calculations for the second sqrt
    double term1_b = (2 * pow(q1*q2 + q4*q5, 2)) / pow(pow(q1, 2) + pow(q4, 2), 2);
    double term2_b = (4 * (pow(q2, 2) + 2*q1*q3 - pow(q4, 2) + pow(q5, 2))) / (3. * (pow(q1, 2) + pow(q4, 2)));
    
    // Calculate the differential term
    double diff_term1 = (-8 * pow(q1*q2 + q4*q5, 3)) / pow(pow(q1, 2) + pow(q4, 2), 3);
    double diff_term2 = (16 * (-(q2*q3) + q4*q5)) / (pow(q1, 2) + pow(q4, 2));
    double diff_term3 = (8 * (q1*q2 + q4*q5) * (pow(q2, 2) + 2*q1*q3 - pow(q4, 2) + pow(q5, 2))) / pow(pow(q1, 2) + pow(q4, 2), 2);
    double diff_numerator = diff_term1 + diff_term2 + diff_term3;
    double diff_denom = 4. * first_sqrt;
    double diff_term = diff_numerator / diff_denom;
    ...
    double second_sqrt_expr_1_2 = term1_b - term2_b - term3_a - term4_a - diff_term; // For functions 1, 2
    double second_sqrt_expr_3_4 = term1_b - term2_b - term3_a - term4_a - diff_term; // For functions 3, 4
    double second_sqrt_expr_5_6 = term1_b - term2_b - term3_a - term4_a + diff_term; // For functions 5, 6
    double second_sqrt_expr_7_8 = term1_b - term2_b - term3_a - term4_a + diff_term; // For functions 7, 8
    
    double second_sqrt_1_2 = sqrt(second_sqrt_expr_1_2);
    double second_sqrt_3_4 = sqrt(second_sqrt_expr_3_4);
    double second_sqrt_5_6 = sqrt(second_sqrt_expr_5_6);
    double second_sqrt_7_8 = sqrt(second_sqrt_expr_7_8);
    
    // Calculate half values for convenience
    double half_first_sqrt = first_sqrt / 2.0;
    double half_second_sqrt_1_2 = second_sqrt_1_2 / 2.0;
    double half_second_sqrt_3_4 = second_sqrt_3_4 / 2.0;
    double half_second_sqrt_5_6 = second_sqrt_5_6 / 2.0;
    double half_second_sqrt_7_8 = second_sqrt_7_8 / 2.0;
    ...
    solutions[0] = -acos(base_term - half_first_sqrt - half_second_sqrt_1_2);
    
    // soln2: sign before acos = +, sign before first sqrt = -, sign before second sqrt = -
    solutions[1] = acos(base_term - half_first_sqrt - half_second_sqrt_1_2);
    
    // soln3: sign before acos = -, sign before first sqrt = -, sign before second sqrt = +
    solutions[2] = -acos(base_term - half_first_sqrt + half_second_sqrt_3_4);
    
    // soln4: sign before acos = +, sign before first sqrt = -, sign before second sqrt = +
    solutions[3] = acos(base_term - half_first_sqrt + half_second_sqrt_3_4);
    
    // soln5: sign before acos = -, sign before first sqrt = +, sign before second sqrt = -
    solutions[4] = -acos(base_term + half_first_sqrt - half_second_sqrt_5_6);
    
    // soln6: sign before acos = +, sign before first sqrt = +, sign before second sqrt = -
    solutions[5] = acos(base_term + half_first_sqrt - half_second_sqrt_5_6);
    
    // soln7: sign before acos = -, sign before first sqrt = +, sign before second sqrt = +
    solutions[6] = -acos(base_term + half_first_sqrt + half_second_sqrt_7_8);
    
    // soln8: sign before acos = +, sign before first sqrt = +, sign before second sqrt = +
    solutions[7] = acos(base_term + half_first_sqrt + half_second_sqrt_7_8);
